...
const ShaderMaterial = ({
  ...
  const getUniforms = (): Record<string, { value: unknown; type: string }> => {
    ...
    for (const uniformName in uniforms) {
      ...
      if (!uniform) continue; // Ensure uniform is defined
      ...
      switch (uniform.type) {
        ...
        case "uniform3f":
          if (Array.isArray(uniform.value) && uniform.value.length === 3) {
            ...
            console.error(
              `Invalid value for '${uniformName}' (expected [number, number, number])`
            );
          }
          break;
        case "uniform1fv":
          if (Array.isArray(uniform.value)) {
            ...
            console.error(
              `Invalid value for '${uniformName}' (expected number[])`
            );
          }
          break;
        case "uniform3fv":
          if (Array.isArray(uniform.value) && Array.isArray(uniform.value[0])) {
            preparedUniforms[uniformName] = {
              value: (uniform.value as number[][]).map((v) =>
                new THREE.Vector3().fromArray(v as [number, number, number])
              ),
              type: "3fv",
            };
          } else {
        ...
        case "uniform2f":
          if (Array.isArray(uniform.value) && uniform.value.length === 2) {
            ...
            console.error(
              `Invalid value for '${uniformName}' (expected [number, number])`
            );
          }
          break;
        default:
